...
function appendRepairInstruction(body, kind) {
  ...
  // Prepend to the current user message content. Kiro rejects a top-level
  // `systemPrompt` field with 400 REQUEST_BODY_INVALID (#2989), so the repair
  // instruction must travel inside user content, never as a system field.
  const uim = repaired?.conversationState?.currentMessage?.userInputMessage;
  if (uim && typeof uim === "object") {
